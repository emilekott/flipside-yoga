	...
	class RevSliderOutput{
		...
		private function putFullWidthVideoLayer($videoData){
			...
			if($videoData["type"] == "youtube"):	//youtube
				...
				?>
				<div class="caption fade fullscreenvideo" data-autoplay="<?php echo $autoplay?>" <?php echo $htmlParams?>><iframe src="http://player.vimeo.com/video/<?echo $videoID?>?title=0&amp;byline=0&amp;portrait=0;api=1" width="100%" height="100%"></iframe></div>
				<?php
			endif;
		}
				...
				<?php foreach($layers as $layer):
					...
					switch($type){
						...
						case "video":
							...
							switch($videoType){
								case "youtube":
									$html = "<iframe src='http://www.youtube.com/embed/{$videoID}?hd=1&amp;wmode=opaque&amp;controls=1&amp;showinfo=0' width='{$videoWidth}' height='{$videoHeight}' style='width:{$videoWidth}px;height:{$videoHeight}px;'></iframe>";
								break;
								case "vimeo":
					...
					$htmlParams = $htmlEnd.$htmlLink.$htmlVideoAutoplay;
				?>
