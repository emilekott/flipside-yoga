...
function aitAdminInit()
{
	...
	if(isset($_POST['resetOptions']) and isset($_SERVER['HTTP_X_REQUESTED_WITH']) and $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest'){
		...
		if(isset($_POST['key'])){
			...
			if($key == AIT_OPTIONS_KEY){
				...
				$c = aitGetThemeDefaultOptions($GLOBALS['aitBrandingConfig']);
	...
	if(isset($_POST['resetOrder']) and isset($_SERVER['HTTP_X_REQUESTED_WITH']) and $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest'){
		...
		if(isset($_POST['orderKey'])){
			...
			$defOptions = aitGetThemeDefaultOptions($GLOBALS['aitThemeConfig']);
	...
	add_filter('admin_footer_text', create_function('', 'echo @$GLOBALS["aitBrandingOptions"]->branding->adminFooterText;'));
}
...
{
	do_action('aitGenerateAdminMenu', $GLOBALS['aitThemeConfig'], $GLOBALS['aitBrandingOptions']);
...
function aitGenerateAdminMenu($config, $brandingOptions)
{
	...
	if(current_user_can('manage_options') && @$GLOBALS['showAdmin']['dashboard'] != 'disabled'){
		$adminTitle = THEME_SHORT_NAME . ' ' . __('Options', THEME_CODE_NAME);
		$title = 'AIT Themes';
		$slug = 'ait-admin';
		$key = 'ait-admin';
	}else{
		if(@$GLOBALS['showAdmin']['backup'] != 'disabled'){
			$adminTitle = $branding->adminTitle;
			$title = $adminTitle;
			$slug = 'ait-admin-backup';
			$key = 'backup';
		}
		if(@$GLOBALS['showAdmin']['skins'] != 'disabled'){
			$adminTitle = $branding->adminTitle;
			$title = $adminTitle;
			$slug = 'ait-admin-skins';
			$key = 'skins';
		}
		if(!@$GLOBALS['aitDisableBranding']){
			if(@$GLOBALS['showAdmin']['branding'] != 'disabled'){
				$adminTitle = $branding->adminTitle;
				$title = $adminTitle;
	...
	if ($slug) {
		add_menu_page(
			$title,
			sprintf( __('AIT Dashboard %s'), "<span class='update-plugins count-{$updateData['counts']['total']}' title='{$updateData['title']}'><span class='update-count'>" . number_format_i18n($updateData['counts']['total']) . "</span></span>" ),
			'unfiltered_html',
			$slug,
...
{
	$branding = @$GLOBALS['aitBrandingOptions']->branding;
	...
	if(isset($GLOBALS['aitThemeConfig'][$key])):
		$page = $GLOBALS['aitThemeConfig'][$key];
	...
	<?php else:
		...
		if($key == 'branding'){
			...
			aitRenderThemeOptionsForm(AIT_BRANDING_OPTIONS_KEY, $key, $GLOBALS['aitBrandingConfig'][$key]);
		}
...
function aitAddAdminMenuSeparator()
{
	if (is_admin()) {
		$index = 0;
		foreach($GLOBALS['menu'] as $offset => $section) {
			if (substr($section[2], 0, 9) == 'separator')
				$index++;

			if ($offset >= AIT_ADMIN_POSITION) {
				$GLOBALS['menu'][AIT_ADMIN_POSITION] = array('', 'read', "separator{$index}", '', 'wp-menu-separator');
				break;
			}
		}
	}
...
function aitCreateNewSkin($page, $tab)
{
	...
	if(isset($_POST['create-new-skin'])){
		...
		$t = aitGetOptionsTypes($GLOBALS['aitThemeConfig'], true);
...
function aitUpdateThemeVersions()
{
	...
	if(function_exists('wp_get_theme')){
		...
		$t = get_theme_data(THEME_STYLESHEET_FILE);
...
{
	$isDisabled = @$GLOBALS['aitDisableBranding'] == true; // @ - doesn't exist
	if($isDisabled)
		return;
