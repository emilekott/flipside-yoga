...
function aitRenderThemeOptionsForm($optionKey, $key, $page)
{
	...
	if($opt == AIT_OPTIONS_KEY and $allOptions === false and defined('ICL_LANGUAGE_CODE')){
		...
		?><p><?php
		_e('For this langauge there is no settings. Please create new settings.');
		?></p>
		...
		_e('For "All languages" creating settings is not allowed.');
		?></p></div><?php
		}
		return;
	}
	...
	if($opt == AIT_BRANDING_OPTIONS_KEY and $allOptions === false){
		...
		_e('These options are not saved in database yet. Save these default values or change it on whatever you want.');
		?></p></div><?php
		$allOptions = array();
	}
...
<div id="ait-options-reseted" class="updated settings-error">
	<p><strong><?php _e('Options reseted.'); ?></strong></p>
</div>
<?php endif; ?>

<?php if(isset($_GET['resetedOrder']) and $_GET['resetedOrder'] == 'true'): ?>
<div id="ait-options-reseted" class="updated settings-error">
	<p><strong><?php _e('Section order reseted!'); ?></strong></p>
</div>
<?php endif; ?>
...
<script>
	jQuery(function() {
		...
		jQuery('#ait-reset-options').click(function(){
			if(confirm('<?php _e('Are you sure you want to reset these settings?') ?>')){
		...
		jQuery('#ait-reset-order').click(function(){
			...
			if(confirm('<?php _e('Are you sure you want to reset this order?') ?>')){
...
<p class="submit">
	<input type="submit" class="button-primary ait-save-options" value="<?php _e('Save options') ?>" />

	<span class="ait-form-actions">
		<?php if($opt == AIT_OPTIONS_KEY): ?> <a href="<?php echo admin_url('admin.php?page=ait-admin-skins&tab=create-new-skin'); ?>" id="ait-new-skin-link" title="<?php _e('Make new skin from these options.', THEME_CODE_NAME) ?>"><?php _e('Make new skin', THEME_CODE_NAME) ?></a><?php endif; ?>
		<a href="#" id="ait-reset-options" title="<?php _e('Reset options to default values from config file.', THEME_CODE_NAME) ?>"><?php _e('Reset Options', THEME_CODE_NAME) ?></a>
	</span>
</p>
...
?>
			...
			<td>
				<?php wp_dropdown_categories(array(
					...
					'show_option_none' => __('None', THEME_CODE_NAME),
					'hide_empty' => 0,
					'show_count' => 1
				)); ?>
			</td>
	...
	<?php /* ***** Counter label ***** */ ?>
					...
					<?php if($value['type'] == 'custom-css-vars'): ?>
						...
						<td colspan="2" class="ait-clone-form-table-delete">
							<a id="ait-clone-form_remove_current" class="button-secondary"><?php _e('Delete', THEME_CODE_NAME) ?></a>
						</td>
						</tr>
					<?php else:?>
...
?>
				...
				<?php break; /* break - generate only once */ endforeach; ?>
					...
					<!-- No forms template -->
					<div id="ait-clone-form_noforms_template"><?php _e('No options.', THEME_CODE_NAME) ?></div>
					<!-- /No forms template-->

				<!-- Controls -->
				<div id="ait-clone-form_controls">
					<a id="ait-clone-form_add" class="button-primary"><?php _e('Add item', THEME_CODE_NAME) ?></a>
					<a id="ait-clone-form_remove_last" class="button-secondary"><?php _e('Remove last item', THEME_CODE_NAME) ?></a>
					<a id="ait-clone-form_remove_all" class="button-secondary"><?php _e('Remove all items', THEME_CODE_NAME) ?></a>
					<span id="ait-clone-form_add_n">
						<input id="ait-clone-form_add_n_input" type="text" size="4">
						<a id="ait-clone-form_add_n_button" class="button-primary"><?php _e('Add', THEME_CODE_NAME) ?></a>
					</span>
				</div>
				<!-- /Controls -->
			</div> <!-- /#ait-clone-form -->
...
function aitSortableSections($sortable, $options, $config, $currentKey, $opt)
{
	...
	<?php if(count($options['sectionsOrder']) > 1):
		?>
	<script src="<?php echo AIT_ADMIN_URL; ?>/gui/jquery-ui-1.8.16.custom.min.js"></script>
	<script>
		jQuery(function() {
			var $flashMsg = jQuery('<span/>', {'class': 'ait-sortable-updated', text: '<?php _e('Order saved'); ?>'}).css('display', 'none');
	...
	<div id="dashboard-widgets-wrap">
		<div id="dashboard-widgets" class="metabox-holder">
			<div class="postbox-container" style="width:100%;">
				<div  class="meta-box-sortables">
					<div class="postbox">
						<h3><span><?php _e('Order of these sections', THEME_CODE_NAME); ?></span></h3>
						<div class="inside">
							<table class="form-table">
	...
	<?php else: ?>
		<div class="error"><p><?php _e('There must be at least two named section in config file for ordering.') ?></p></div>
	<?php endif; endif;
}
...
function aitRenderOrderType($order, $sections, $optionCurrentKey, $optionKey, $opt, $metaName)
{
	...
	<script>
		jQuery(function() {
			var $flashMsg = jQuery('<span/>', {'class': 'ait-order-updated', text: '<?php _e('Order saved'); ?>'}).css('display', 'none');
	...
	?>
		...
		<?php else: ?>
		<div class="error"><p><?php _e('There must be at least two section in config file for ordering.') ?></p></div>
	<?php endif;
}