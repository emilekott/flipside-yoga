...
class WpLatteMacros extends NMacroSet
{
	...
	public static function install(NParser $parser)
	{
		...
		$me->addMacro('editPostLink', 'edit_post_link(__("Edit"), "<span class=\"edit-link\">", "</span>", %node.word);');

		$me->addMacro('editCommentLink', 'WpLatteFunctions::editCommentLink(__("Edit"), %node.word, "<span class=\"edit-link\">", "</span>");');
		...
		$me->addMacro('listComments',
			...
			$iterations = 0; foreach ($iterator = $_l->its[] = new NSmartCachingIterator($a["comments"]) as $comment):
				if ($comment->depth > $depth){
					...
					echo str_repeat("</li></ul></li>", $depth - $comment->depth);
					$depth = $comment->depth;
				}
			',
			'
