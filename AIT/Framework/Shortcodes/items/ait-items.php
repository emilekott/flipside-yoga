<?php
function get_custom_item( $params ) {
    ...
    if ($id) {
        ...
        $latest_post = query_posts( 'post_type=ait-item&p=' . $id );
        wp_reset_query();
    }
    ...
    if($display == "1"){
        ...
        $category = get_the_category_list( ', ', $parents = '', $latest_post['0']->ID );
          ...
          if (has_post_thumbnail( $latest_post['0']->ID )) {
              ...
              $result .= '<img src="'.TIMTHUMB_URL . '?src='.wp_get_attachment_url( get_post_thumbnail_id($latest_post['0']->ID) ).'&amp;w=200&amp;h=80" alt="'.$latest_post['0']->post_title.'" />';
              $result .= '</a>';
              $result .= '</div>';
              $result .= '</div>';
              $result .= '</header>';
...
add_shortcode( "get_item", "get_custom_item" );
...
function get_custom_items( $params ) {
    ...
    $latest_posts = query_posts( array( 'post_type' => 'ait-item', 'orderby' => 'menu_order', 'order' => 'ASC', 'posts_per_page' => $number )  );
    wp_reset_query();
    ...
    if($display == "1"){
        ...
        foreach ($latest_posts as $key => $latest_post) {
          ...
          if (has_post_thumbnail( $latest_post->ID )) {
              ...
              $result .= '<div class="entry-thumb-img">';
              $result .= '<a href="' . $post_link . '">';
              $result .= '<img src="'.TIMTHUMB_URL . '?src='.wp_get_attachment_url( get_post_thumbnail_id($latest_post->ID) ).'&amp;w=306&amp;h=120" alt="'.$latest_post->post_title.'" />';
              $result .= '</a>';
              $result .= '</div>';
              $result .= '</div>';
              $result .= '</header>';
...
add_shortcode( "get_item_category", "get_category_items" );
