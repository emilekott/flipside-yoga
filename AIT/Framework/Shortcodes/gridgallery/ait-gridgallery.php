...
function ait_gridgallery( $params ) {
	...
	switch($display)
  {
    ...
    foreach($portfolio_images as $counter=>$item){
      ...
      {
        global $wpdb;
        $sql = "SELECT meta_value FROM wp_postmeta WHERE post_id=".$item->ID." AND meta_key LIKE '%_ait%'";
        ...
        $result .= '<li class="item left tile-'.$i.' '.$item->tags.' '.$itemCategories.' itemType-'.$itemMetaValue['itemType'].'" data-id="id-'.$i.'">';
