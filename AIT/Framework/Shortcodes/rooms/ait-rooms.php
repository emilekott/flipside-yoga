    ...
    if($display == "1"){
        ...
        foreach ($latest_posts as $key => $latest_post) {
          ...
          if (has_post_thumbnail( $latest_post->ID )) {
              ...
              $result .= '<div class="entry-thumb-img">';
              $result .= '<a href="' . $post_link . '">';
              $result .= '<img src="'.get_bloginfo('template_url').'/AIT/Framework/Libs/timthumb/timthumb.php?src='.wp_get_attachment_url( get_post_thumbnail_id($latest_post->ID) ).'&amp;w=309&amp;h=120" alt="'.$latest_post->post_title.'" />';
              $result .= '</a>';
              $result .= '</div>';
              $result .= '</div>';
              $result .= '</header>';
